...
tr=10:1:40; %rango temp radiante media
met=[1,1.6,2.4,3.2,4]; %rango actividad metabólica
clo=[0.1,0.5,1,1.5,2]; %rango aislamiento de la ropa
ta=[12.5,17.5,25,32.5,40]; %rango temp aire
vair=[0.1,0.25,0.5,0.75,1];%rango velocidad del aire
rh=[35,40,50,60,70]; %rango humedad relativa
...
tcl(1,j)=fsolve(@funtlc_tr,tclini(1,i),options,vair(1,i),j,i);
...
pmv6(1,j)=3.96*0.00000001.*fcl(1,i).*(((tcl(1,j)+273)).^4-((tr(1,j)+273)).^4);%heat loss by radiation
...
plot(tr,pmv_a(1,:));
  ...
  hold on;
  plot(tr,pmv_a(5,:))
  title('Temperatura radiante media/PMV')
    xlabel('ºC')
    ylabel('PMV')
