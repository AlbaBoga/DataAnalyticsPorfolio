...
for n = 1:length(mat)
    load(mat(n).name);
    if mat(n).bytes>443%elimina los que están vacíos
...
trsentado(trsentado<15)=[];
trpie(trpie<0)=[];
trpie(trpie<15)=[];
%Hago que el vector de entrada de mi sitema (tair) 
%y el vector de salida tengan las mismas dimensiones
for i=1:1:length(tair)
    trsentado_new(i,1)=trsentado(i,1);
end
for i=1:1:length(tair)
    trpie_new(i,1)=trpie(i,1);
end
